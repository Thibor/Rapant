...
static string MoveToUci(int move) {
	...
	string uci = SQSTR[sqS] + SQSTR[sqD];
	if (promo < KING)
		uci += "pnbrqk"[promo];
	return uci;
}
...
static int UciToMove(string s) {
	...
	int Src = SQ(s[0] - 'a', 7 - (s[1] - '1')), Dst = SQ(s[2] - 'a', 7 - (s[3] - '1')), Upgrade = PT_NB;
...
struct Movelist
{
	...
	void Generate(Position& pos, int onlyCapture) {
		...
		for (int sq = 20; sq < 100; sq++)
			...
			case KING:
				GenPieceMoves(K_dirs, 0, 8, sq, pos, color);
...
static U64 GetHash(const Position& pos) {
	U64 CheckSum = pos.color;
	for (int index = 21; index <= 99; index++) {
		int nPiece = pos.board[index];
		if (nPiece == INVALID) continue;
		if (nPiece != EMPTY) CheckSum ^= keys[index << 4 | (nPiece & 0xf)];
	}
	return CheckSum;
}
